class App {
	...
	initPhotographers(photographers) {
		photographers.forEach((photographer, i) => {
			this._Photographers.push(new Photographer(photographer));
			this._Photographers[i].init();
		});
	}

	async init() {
		this._data = await this.Api.getData();
		this.initPhotographers(this._data.photographers);
